...
//            emptyFormat: function(query) {return "没有搜索结果"},
    ...
    S.augment(DropList, S.EventTarget, /** @lends DropList.prototype*/{
        ...
        _bindInput: function(elInput) {
            ...
            E.on(elInput, 'blur', function() {
                ...
                if(data === undefined && cfg.freedom && inputText !== EMPTY) {
                    ...
                    // data = cfg.custom ? self.getInputData() : self.getCustomData();
                }

                self.fire('blur', { data: {text: inputText, value: inputText }});
            ...
            E.on(elInput, 'keyup', function(ev) {
                ...
                if(!kw) {
                    view.render(datalist.getDataSource());
                    self.show();
                    var obj = { text: kw, value: kw };
                    self.fire('change', {data: obj});
                    return;
                }
        ...
        _syncFilter: function(kw, callback) {
            ...
            if( result.length == 0 && datalist.cfg.custom ){
                var obj = { text: kw, value: kw };
                self.fire('change', {data: obj});
            }
